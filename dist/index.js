...
const getEventSource = get$1('Records[0].eventSource');
...
var func$1 = convert_1('isFunction', isFunction_1, _falseOptions);
...
const withAwsRegion = (headers) => {
  const addHeaders = (handler, next) => {
    if(isHttp(handler.event)) {
      ...
      const newHeaders = isFunction$1(headers) ? headers(handler) : headers;
...
function withDynamoDbRecordFilter(eventNames) {
  eventNames = isArray$1(eventNames) ? eventNames : [eventNames];
...
function withEnvironmentVariable(variables) {
  return {
    before: (handler, next) => {
      ...
      const env$$1 = variables 
        ? pick$1(variables, process.env) 
        : process.env;

      context.env = Object.assign({}, context.env, env$$1);
...
const validate$1 = (content, schema, options) => {
  ...
  return {
    before: (handler, next) => {
      ...
      schemas.forEach(name => {
        event[name] = validate$1(event[name], config[name], joiOptions);
      });
...
var func$4 = convert_1('difference', difference_1);
...
var func$5 = convert_1('isNil', isNil_1, _falseOptions);
...
class Cache {
  ...
  getKey(key) {
    ...
    if(isNil$1(obj) || obj._expires > Date.now()) {
...
const getFromSSM = (parameters) => {
  const options = {
    Names: isArray$1(parameters) ? parameters : [parameters],
    WithDecryption: true
  };
...
var func$6 = convert_1('isObject', isObject_1, _falseOptions);
